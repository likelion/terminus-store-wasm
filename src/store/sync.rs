...
impl SyncStoreLayer {
    ...
    pub fn squash_upto(&self, upto: &SyncStoreLayer) -> Result<SyncStoreLayer, io::Error> {
        self.inner.squash_upto(&upto.inner).map(SyncStoreLayer::wrap)
    }
    ...
    pub fn triple_addition_exists(
        ...
        self.inner.triple_addition_exists(subject, predicate, object)
    }
    ...
    pub fn triple_additions_s(&self, subject: u64) -> io::Result<Box<dyn Iterator<Item = IdTriple> + Send>> {
        self.inner.triple_additions_s(subject)
    }

    pub fn triple_removals_s(&self, subject: u64) -> io::Result<Box<dyn Iterator<Item = IdTriple> + Send>> {
        self.inner.triple_removals_s(subject)
    }

    pub fn triple_additions_sp(&self, subject: u64, predicate: u64) -> io::Result<Box<dyn Iterator<Item = IdTriple> + Send>> {
        self.inner.triple_additions_sp(subject, predicate)
    }

    pub fn triple_removals_sp(&self, subject: u64, predicate: u64) -> io::Result<Box<dyn Iterator<Item = IdTriple> + Send>> {
...
impl Layer for SyncStoreLayer {
    fn name(&self) -> [u32; 5] { self.inner.name() }
    fn parent_name(&self) -> Option<[u32; 5]> { self.inner.parent_name() }
    fn node_and_value_count(&self) -> usize { self.inner.node_and_value_count() }
    fn predicate_count(&self) -> usize { self.inner.predicate_count() }
    fn subject_id(&self, subject: &str) -> Option<u64> { self.inner.subject_id(subject) }
    fn predicate_id(&self, predicate: &str) -> Option<u64> { self.inner.predicate_id(predicate) }
    fn object_node_id(&self, object: &str) -> Option<u64> { self.inner.object_node_id(object) }
    fn object_value_id(&self, object: &TypedDictEntry) -> Option<u64> { self.inner.object_value_id(object) }
    fn id_subject(&self, id: u64) -> Option<String> { self.inner.id_subject(id) }
    fn id_predicate(&self, id: u64) -> Option<String> { self.inner.id_predicate(id) }
    fn id_object(&self, id: u64) -> Option<ObjectType> { self.inner.id_object(id) }
    fn id_object_is_node(&self, id: u64) -> Option<bool> { self.inner.id_object_is_node(id) }
    fn triple_exists(&self, subject: u64, predicate: u64, object: u64) -> bool { self.inner.triple_exists(subject, predicate, object) }
    fn triples(&self) -> Box<dyn Iterator<Item = IdTriple> + Send> { self.inner.triples() }
    fn triples_s(&self, subject: u64) -> Box<dyn Iterator<Item = IdTriple> + Send> { self.inner.triples_s(subject) }
    fn triples_sp(&self, subject: u64, predicate: u64) -> Box<dyn Iterator<Item = IdTriple> + Send> { self.inner.triples_sp(subject, predicate) }
    fn triples_p(&self, predicate: u64) -> Box<dyn Iterator<Item = IdTriple> + Send> { self.inner.triples_p(predicate) }
    fn triples_o(&self, object: u64) -> Box<dyn Iterator<Item = IdTriple> + Send> { self.inner.triples_o(object) }
    fn clone_boxed(&self) -> Box<dyn Layer> { Box::new(self.clone()) }
    fn triple_addition_count(&self) -> usize { self.inner.triple_addition_count() }
    fn triple_removal_count(&self) -> usize { self.inner.triple_removal_count() }
    fn all_counts(&self) -> LayerCounts { self.inner.all_counts() }
    fn single_triple_sp(&self, subject: u64, predicate: u64) -> Option<IdTriple> { self.inner.single_triple_sp(subject, predicate) }
}
...
impl SyncNamedGraph {
    fn wrap(inner: NamedGraph) -> Self { Self { inner } }
    pub fn name(&self) -> &str { self.inner.name() }
    pub fn head_version(&self) -> io::Result<(Option<SyncStoreLayer>, u64)> {
        self.inner.head_version().map(|(layer, version)| (layer.map(SyncStoreLayer::wrap), version))
    }
...
impl SyncStore {
    pub fn wrap(inner: Store) -> Self { Self { inner } }
    ...
    pub fn get_layer_from_id(&self, layer: [u32; 5]) -> Result<Option<SyncStoreLayer>, io::Error> {
        self.inner.get_layer_from_id(layer).map(|l| l.map(SyncStoreLayer::wrap))
    }
    pub fn create_base_layer(&self) -> Result<SyncStoreLayerBuilder, io::Error> {
        self.inner.create_base_layer().map(SyncStoreLayerBuilder::wrap)
    }
    pub fn merge_base_layers(&self, layers: &[[u32; 5]], temp_dir: &Path) -> Result<[u32; 5], io::Error> {
        self.inner.merge_base_layers(layers, temp_dir)
    }
    pub fn export_layers(&self, layer_ids: Box<dyn Iterator<Item = [u32; 5]> + Send>) -> io::Result<Vec<u8>> {
        self.inner.layer_store.export_layers(layer_ids)
    }
    pub fn import_layers(&self, pack: &[u8], layer_ids: Box<dyn Iterator<Item = [u32; 5]> + Send>) -> io::Result<()> {
...
mod tests {
    ...
    fn create_and_manipulate_sync_memory_database() {
        ...
        builder.add_value_triple(ValueTriple::new_string_value("pig", "says", "oink")).unwrap();
