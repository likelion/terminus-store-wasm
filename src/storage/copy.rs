...
use tdb_succinct_wasm::storage::copy_file;
...
impl<F1: 'static + FileLoad + FileStore> BaseLayerFiles<F1> {
    pub fn copy_from<F2: 'static + FileLoad + FileStore>(
        ...
        self.id_map_files.copy_from(&from.id_map_files)?;
        copy_file(&from.subjects_file, &self.subjects_file)?;
        copy_file(&from.objects_file, &self.objects_file)?;
...
impl<F1: 'static + FileLoad + FileStore> ChildLayerFiles<F1> {
    pub fn copy_from<F2: 'static + FileLoad + FileStore>(
        ...
        copy_file(&from.pos_subjects_file, &self.pos_subjects_file)?;
        copy_file(&from.pos_objects_file, &self.pos_objects_file)?;
        ...
        copy_file(&from.neg_subjects_file, &self.neg_subjects_file)?;
        copy_file(&from.neg_objects_file, &self.neg_objects_file)?;
